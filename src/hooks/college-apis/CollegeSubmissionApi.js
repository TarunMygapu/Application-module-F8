...
export const mapCollegeApplicationSaleCompleteToPayload = (formData, paymentData, detailsObject, activeTab) => {
  ...
  console.log('  - siblings in payload:', payload.siblings);
  console.log('  - siblings count in payload:', payload.siblings?.length || 0);
  console.log('  - studAdmsNo:', payload.studAdmsNo);
  console.log('  - firstName:', payload.firstName);
  console.log('  - lastName:', payload.lastName);
...
export const submitCollegeApplicationSaleComplete = async (payload) => {
  try {
    ...
    console.log('🌐 Submitting to:', fullUrl);
    console.log('📦 Payload size:', JSON.stringify(payload).length, 'bytes');
    console.log('🔍 ===== FINAL PAYLOAD BEFORE SUBMISSION =====');
    console.log('  - payload.siblings:', payload.siblings);
    console.log('  - payload.siblings type:', typeof payload.siblings);
    console.log('  - payload.siblings is array?', Array.isArray(payload.siblings));
    console.log('  - payload.siblings length:', payload.siblings?.length || 0);
    if (payload.siblings && payload.siblings.length > 0) {
      console.log('  - First sibling in payload:', payload.siblings[0]);
      console.log('  - All siblings:', JSON.stringify(payload.siblings, null, 2));
    }
