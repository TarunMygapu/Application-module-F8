...
export const getApplicationStatusData = async (empId, campusCategory) => {
  try {
    const response = await apiClient.get(`/application-status/status-by-role?empId=${empId}&category=${campusCategory}`);
...
export const useApplicationStatusData = (empId,campusCategory) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await getApplicationStatusData(empId, campusCategory);
