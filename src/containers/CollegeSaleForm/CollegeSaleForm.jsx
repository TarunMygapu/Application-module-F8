...
const CollegeSalePage = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Set city after district is set so dropdown options are available
      const cityNameRootValue = sourceData.cityName || sourceData.city || sourceData.city_name || sourceData.addressCityName;
      if (cityNameRootValue) {
        // Use setTimeout to ensure city is set after district dropdown is populated and city API loads
        // The AddressInformation component will sync cityId once city options are loaded
        setTimeout(() => {
          formik.setFieldValue("city", cityNameRootValue);
          console.log("✅ Auto-populated city (root level):", cityNameRootValue);
        }, 300);
      }
