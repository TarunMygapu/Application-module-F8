...
const PersonalInformationForSchool = ({ isEditMode = false }) => {
  ...
  const admissionTypeNameToId = useMemo(() => {
    ...
    admissionData?.forEach((a) => {
      // Handle different possible field names from API
      const name =
        a.name || a.admissionTypeName || a.admission_type_name || a.typeName;
    ...
    return personalInfoFields.reduce((acc, field) => {
      ...
      if (
        ...
        (f.name === "firstName" ||
          ...
          f.name === "aaparNo" ||
          f.name === "aadharCardNo" ||
          f.name === "quotaAdmissionReferredBy" ||
          f.name === "employeeId" ||
          f.name === "admissionType")
      ) {
        f.disabled = true;
        f.readOnly = true;
      }
  ...
  useEffect(() => {
    if (
      values.quotaAdmissionReferredBy &&
      (!values.quotaId || values.quotaId === 0)
    ) {
      ...
      if (quotaNameToId.has(values.quotaAdmissionReferredBy)) {
        ...
        const quotaUpper = String(values.quotaAdmissionReferredBy)
          .toUpperCase()
          .trim();
                  ...
                  renderField(fname, fieldMap, {
                    ...
                    disabled:
                      ...
                      (fname === "firstName" ||
                        ...
                        fname === "aaparNo" ||
                        fname === "aadharCardNo" ||
                        fname === "quotaAdmissionReferredBy" ||
                        fname === "employeeId" ||
                        fname === "admissionType")
                        ? true
                        : undefined,
                    onChange:
                      ...
                      (fname === "firstName" ||
                        ...
                        : (e) => {
                            ...
                            if (
                              fname === "admissionType" &&
                              selectedValue === values.admissionType
                            ) {
                              return;
                            }
                            if (
                              fname === "quotaAdmissionReferredBy" &&
                              selectedValue === values.quotaAdmissionReferredBy
                            ) {
                              return;
                            }
                            ...
                            if (fname === "quotaAdmissionReferredBy") {
                              ...
                              if (selectedValue.includes(" - ")) {
                                ...
                                // Lookup by name only
                                employeeId =
                                  employeeNameToId.get(selectedValue);
                                // Update the value to "name - id" format
                                const employee = employeesData?.find((e) => {
                                  const name =
                                    e.name || e.employeeName || e.employee_name;
                              ...
                              if (admissionTypeNameToId.has(selectedValue)) {
                                const id =
                                  admissionTypeNameToId.get(selectedValue);
                                setFieldValue("appTypeId", id);
                              }
                            }
                    ...
                    error:
                      (touched[fname] || formik.submitCount > 0) &&
                      formik.errors[fname]
                        ? formik.errors[fname]
                        : null,
                  })}
