...
const ApplicationStatus = () => {
  ...
  const category = localStorage.getItem('campusCategory') ; // Default to school
  ...
  // Handle search change - only allow digits, max 9 digits
  const handleSearchChange = (e) => {
    ...
    if (applicationNo) {
      ...
      if (displayStatus === "Sold" || displayStatus === "Confirmed") {
        ...
        if (normalizedCategory === 'college') {
          ...
          route = "school-confirmation"; // School confirmation route
        }
      } else {
        // For sale: use category-specific route
        if (normalizedCategory === 'college') {
          route = "college-sale";
        } else {
          route = "school-sale"; // School sale route
        }
      }
     
      // Navigate with applicationData in state (not initialValues)
      navigate(`/scopes/application/status/${applicationNo}/${route}`, {
        state: {
          applicationData: item, // Pass as applicationData to match what confirmation container expects
        },
      });
    }
  };
