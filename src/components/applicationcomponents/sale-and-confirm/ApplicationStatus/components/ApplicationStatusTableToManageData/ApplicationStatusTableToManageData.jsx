...
import styles from '../../../../../../widgets/ApplicationStatusDataTable/ApplicationStatusDataTable.module.css';
import './ApplicationStatusTableToManageData.module.css';
import { useApplicationStatusData } from '../../../../../../queries/applicationqueries/application-status-apis/ApplicationStatusApis';
...
const mapStatusToDisplayStatus = (status) => {
    if (!status) return '';
    const normalized = normalizeStatus(status);
    // If status is in config, use the configured display status
    if (statusConfig[normalized]?.displayStatus) {
        return statusConfig[normalized].displayStatus;
    }
    // Fallback: Convert to Title Case for unknown statuses
    return status
        .split(' ')
...
const ApplicationStatusTableToManageData = ({
    ...
    navigate,
    handleNavigateToSalePage,
    employeeCampusId // Default to 4004, will be replaced with login data later
}) => {
    const [internalPageIndex, setInternalPageIndex] = useState(0);
    ...
    const empId = employeeCampusId; // Map prop name to API parameter name
    const { data: apiData, loading, error } = useApplicationStatusData(empId, campusCategory);
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: ({ getValue }) => {
                const status = getValue();
                const badgeClass = getStatusBadgeClass(status);
                // Use statusConfig to get proper display status (e.g., "fast sale" -> "Fast Sold")
                const displayStatus = mapStatusToDisplayStatus(status);
                return (
                    <span className={`${styles.Application_Status_Table_status_badge} ${badgeClass}`}>
                        {displayStatus}
                    </span>
