...
const ParentInformationForSchool = ({ isEditMode = false }) => {
  ...
  const fieldMap = useMemo(() => {
    return parentInfoFields.reduce((acc, f) => {
      ...
      // Disable fatherName and fatherMobile in edit mode
      if (isEditMode && (f.name === "fatherName" || f.name === "fatherMobile" || f.name === "mobileNumber")) {
        field.disabled = true;
        field.readOnly = true;
      }
            ...
            {row.fields.map((fname) => (
              <div key={fname} className={styles.clgAppSaleFieldCell}>
                {renderField(fname, fieldMap, {
                  value: values[fname] ?? "",
                  disabled: (isEditMode && (fname === "fatherName" || fname === "fatherMobile" || fname === "mobileNumber")) ? true : undefined,
                  onChange: (isEditMode && (fname === "fatherName" || fname === "fatherMobile" || fname === "mobileNumber")) ? undefined : (e) => {
