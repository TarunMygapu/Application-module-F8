...
export const SelectedEntityProvider = ({ children }) => {
  const [selectedEntity, setSelectedEntity] = useState({
    ...
    name: null,
    type: null, // 'zone', 'dgm', or 'campus'
    cmpsId: null, // Campus ID for DGM employees (used for graph API)
  });
 
  const [selectedAmount, setSelectedAmount] = useState(null); // Amount filter for graphs

  const selectEntity = (id, name, type, cmpsId = null) => {
    console.log("=== CONTEXT: selectEntity called ===");
    console.log("ID:", id, "Name:", name, "Type:", type, "CmpsId:", cmpsId);
    setSelectedEntity({ id, name, type, cmpsId });
  };

  const clearSelection = () => {
    console.log("=== CONTEXT: clearSelection called ===");
    setSelectedEntity({ id: null, name: null, type: null, cmpsId: null });
  };
